// src/pages/Boveda.jsx - VERSIÓN CON FILTRO DE GÉNEROS
...
const Boveda = () => {
  ...
  const [genreFilter, setGenreFilter] = useState('all'); // NUEVO: Filtro de géneros
  ...
  }, [adminProducts, searchTerm, activeFilter, genreFilter, sortBy]); // Añadí genreFilter aquí
  ...
  // NUEVO: Obtener lista única de géneros
  const allGenres = [...new Set(adminProducts.map(p => p.genre).filter(g => g))].sort();
          ...
          {filteredProducts.map(product => (
            <div key={product.id} className="col">
              <Link to={`/album/${product.id}`} className="text-decoration-none">
                <div className={`format-item-card clickable-card ${product.heritage ? 'heritage-card' : ''}`}>
                  <div 
                    ...
                    <div className="format-item-footer">
                      <div>
                        ...
                        <small className="d-block text-light">
                          Stock: <span className={product.stock < 5 ? 'text-danger' : 'text-success'}>
